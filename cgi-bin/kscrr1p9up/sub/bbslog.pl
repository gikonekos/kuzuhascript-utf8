...
sub prtoldlog {
	...
	# [2026-07-16] 脆弱性対策: $FORM{'e'}は外部入力のため、パストラバーサル
	# （../等）およびPerlの2引数open()仕様によるパイプ経由のコマンド実行を
	# 防ぐため、想定される過去ログファイル名の形式に厳密に一致するかを検証する
	unless ( $FORM{'e'} =~ /^(\d{6,8}\.(?:html|dat))$/ ) {
		&prterror ( '不正なログファイル名です。' );
	}
	open ( OLDLOG, '<', "$oldlogfiledir$1" ) || &prterror ( "$FORM{'e'}を開けませんでした" );
	eval 'flock ( OLDLOG, 1 )';
	seek ( OLDLOG, 0, 0 );
	@oldlog = <OLDLOG>;
	eval 'flock ( OLDLOG, 8 )';
